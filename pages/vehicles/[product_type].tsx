...
export default function ProductType({
  ...
  const products = _products.filter((p) => p.gallery?.at(0))
  ...
  products.forEach((product) => {
    if (product.brand_name && !brands.includes(product.brand_name.toLowerCase())) {
      brands.push(product.brand_name.toLowerCase())
    }
  })
  const [minValue, setMinValue] = useState(0)
  const [_minValue, _setMinValue] = useState(30000)
  const [maxValue, setMaxValue] = useState(1000000)
  const [_maxValue, _setMaxValue] = useState(350000)
  ...
  const filteredProducts = products.filter((product) => {
    if (selectedBrand && product?.brand_name?.toLowerCase() !== selectedBrand) return false
    if (selectedTransmission && product?.transmission_type?.toLowerCase() !== selectedTransmission)
      return false
    if (selectedDestination && product?.destination?.toLowerCase() !== selectedDestination)
      return false
    if ((product?.price_per_day || 0) < minValue) return false
    if ((product?.price_per_day || 0) > maxValue) return false
    return true
  })
  ...
  return (
    <div>
      <Head>
        <title>
          {localizedString(product_category.title, locale)?.toUpperCase() || 'All'} Cars
        </title>
        <meta
          name="title"
          content={localizedString(product_category.title, locale)?.toUpperCase()}
        />
        <meta name="description" content={localizedString(product_category.description, locale)} />
      ...
      <Wrapper>
        <Container>
          <div className="pt-[100px] pb-20">
            <h1 className="mb-3 text-[49px] font-bold">
              {localizedString(product_category.title, locale)?.toUpperCase()}
            </h1>
            <div className="max-w-[610px] font-inter text-[#8a8a8a]">
              {localizedString(product_category.description, locale)}
            </div>
          </div>
          ...
          <div className="my-10 flex flex-row justify-between gap-3 lg:flex-row">
            <h1 className="mt-2">
              {
                <Link className="underline underline-offset-4" href="/" locale={locale}>
                  Home
                </Link>
              }{' '}
              /{' '}
              {
                <Link className="underline underline-offset-4" href="/vehicles" locale={locale}>
                  Products
                </Link>
              }{' '}
              / <span className="text-yellow-400">{product_type?.toUpperCase() || 'All'} Cars</span>
            </h1>
            ...
            >
              ...
              <div className="mt-20">
                <Menu as={'div'} className="relative m-[20px] flex flex-col rounded-[4px]">
                  <Menu.Button>
                    {({ open }) => (
                      <a
                        className={
                  ...
                  <Menu.Items className="absolute right-0 z-20 mt-[60px] flex w-[275px]  flex-col bg-gray-100 p-4  py-10 drop-shadow-lg ">
                    <ReactSlider
                      ...
                      defaultValue={[_minValue, _maxValue]}
            ...
            <div className=" hidden items-center space-x-1 md:flex lg:flex">
              ...
              <Menu as={'div'} className="relative flex flex-col px-1">
                <Menu.Button>
                  {({ open }) => (
                    <a
                      className={
          ...
          <div className="grid grid-flow-row gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {filteredProducts
              ...
              .map((item, index) => (
                <CarCard key={index} product={item} />
              ))}
          </div>
...
export const getStaticPaths: GetStaticPaths = async ({ locales }) => {
  const product_categories = await client.fetch(`*[_type == "productCategory"]`)
  const paths: any[] = []
  locales?.forEach((locale) => {
    product_categories
      .map((cat: any) => ({ params: { product_type: cat.product_category_id }, locale }))
      .forEach((item: any) => paths.push(item))
  })
  return {
    paths,
    fallback: false,
...
export const getStaticProps: GetStaticProps<PageProps> = async ({ params, locale }) => {
  const product_categories = await client.fetch(
    `*[_type == "productCategory" && product_category_id=="${params?.product_type}"]`
  )
  const products = await client.fetch(
    `*[_type == "product" && product_type=="${params?.product_type}"]`
  )
  ...
  return {
    props: {
      ...
      product_type: typeof params?.product_type === 'string' ? params?.product_type : '',
      locale: locale || '',
    },
  }
}
